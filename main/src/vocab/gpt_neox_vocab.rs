...
/// # GPTNeoX Vocab
/// Vocabulary for GPTNeoX tokenizer. Contains the following special values:
/// - BOS token
/// - EOS token
/// - UNK token
///
...
impl Vocab for GptNeoXVocab {
    ...
    fn from_file<P: AsRef<Path>>(path: P) -> Result<GptNeoXVocab, TokenizerError> {
...
mod tests {
    ...
    fn test_create_vocab() {
        ...
        //        When
        let gpt_neox_vocab = GptNeoXVocab {
            values,
            indices,
            special_token_map,
            special_values,
        ...
        // Then
        assert_eq!(gpt_neox_vocab.get_unknown_value(), "<|endoftext|>");
        assert_eq!(
            gpt_neox_vocab.special_token_map.bos_token.as_ref().unwrap(),
            "<|endoftext|>"
        );
        assert_eq!(
            gpt_neox_vocab.special_token_map.eos_token.as_ref().unwrap(),
            "<|endoftext|>"
        );
    ...
    fn test_create_object_from_file() -> anyhow::Result<()> {
        ...
        //        When
        let gpt_neox_vocab = GptNeoXVocab::from_file(&path)?;

        //        Then
        assert_eq!(gpt_neox_vocab.special_token_map.unk_token, "<|endoftext|>");
        assert_eq!(gpt_neox_vocab.values, target_values);
        assert_eq!(gpt_neox_vocab.special_values, special_values);
        drop(path);
        Ok(())
    }
    ...
    fn test_create_object_from_file_without_unknown_token() {
        ...
        //        When & Then
        let _gpt_neox_vocab = GptNeoXVocab::from_file(&path).unwrap();
    }
    ...
    fn test_encode_tokens() -> anyhow::Result<()> {
        ...
        //        When & Then
        assert_eq!(gpt_neox_vocab.token_to_id("hello"), 1);
        assert_eq!(gpt_neox_vocab.token_to_id("world"), 0);
        assert_eq!(gpt_neox_vocab.token_to_id("!"), 3);
        assert_eq!(gpt_neox_vocab.token_to_id("<|endoftext|>"), 2);
        assert_eq!(gpt_neox_vocab.token_to_id("oov_value"), 2);
    ...
    fn test_decode_tokens() -> anyhow::Result<()> {
        ...
        //        When & Then
        assert_eq!(gpt_neox_vocab.id_to_token(&(1_i64)), "hello");
        assert_eq!(gpt_neox_vocab.id_to_token(&(0_i64)), "world");
        assert_eq!(gpt_neox_vocab.id_to_token(&(3_i64)), "!");
        assert_eq!(gpt_neox_vocab.id_to_token(&(2_i64)), "<|endoftext|>");
        drop(path);
        Ok(())
    }
}
